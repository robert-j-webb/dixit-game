...
export function ImageGeneration() {
  ...
  const [selectedModels, setSelectedModels] = useState<string[]>(() =>
    getRandomModels(imageModels)
  );
  const provider = useProvider();
  const [isGenerating, setIsGenerating] = useState(false);
  const [isGenerationComplete, setIsGenerationComplete] = useState(false);
  const [generatedImages, setGeneratedImages] = useState<
    Array<{ src: string; model: string; score?: number }>
  >([]);
    ...
    await Promise.all(
      selectedModels.map(async (model) => {
        try {
          ...
          const fallbackImage = {
            src: `/placeholder.svg?height=400&width=400&query=${encodeURIComponent(
  ...
  const handleShuffleModels = () => {
    if (!isGenerating) {
      setSelectedModels(getRandomModels(imageModels));
      setGeneratedImages([]);
      setCurrentStep(0);
      setIsGenerationComplete(false);
    }
